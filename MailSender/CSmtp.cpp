	...
	const char* password/*=NULL*/)
{
	...
	try
	{
		...
		{
			sockAddr.sin_family = AF_INET;
			sockAddr.sin_port = htons(m_iProxyPort);
			if ((sockAddr.sin_addr.s_addr = inet_addr(m_sProxyHost.c_str())) == INADDR_NONE)
			{
				LPHOSTENT host;

				host = gethostbyname(m_sProxyHost.c_str());
				if (host)
					memcpy(&sockAddr.sin_addr, host->h_addr_list[0], host->h_length);
				else
				{
		...
		if (m_sProxyHost.length())
		{
			m_proxy.setProxyInfo(2, m_sProxyHost, m_iProxyPort, m_sProxyUsername, m_sProxyPassword);
			if (0 != m_proxy.ConnectServer(hSocket, szServer, nPort_))
			{
#ifdef LINUX
				close(hSocket);
#else
...
void CSmtp::FormatHeader(char* header)
{
	...
	if (m_sMessageId.size())
	{
		strcat(header, "Message-ID: ");
		strcat(header, m_sMessageId.c_str());
		strcat(header, "\r\n");
	}
...
void CSmtp::setProxy(const char* proxyHost, unsigned short proxyPort, const char* proxyUsername, const char* proxyPasswrod)
{
	m_sProxyHost.clear();
	if (proxyHost)
		m_sProxyHost.append(proxyHost);
	m_iProxyPort = proxyPort;
	m_sProxyUsername.clear();
	if (proxyUsername)
		m_sProxyUsername.append(proxyUsername);
	m_sProxyPassword.clear();
	if (proxyPasswrod)
		m_sProxyPassword.append(proxyPasswrod);
}

void CSmtp::setSendTime(time_t time)
{
	m_sendTime = time;
}

void CSmtp::setMessageId(const char* messageId)\
{
	m_sMessageId.clear();
	if (messageId)
		m_sMessageId.append(messageId);
}